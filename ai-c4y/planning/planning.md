...
> may read (and, when `writable`, write), a set of configurable **`memory_data_stores`** (SQLite
> knowledge tables it progressively **distills**), and a set of **delegates** — themselves
...
src/framework/
  ...
  config/
    load.py                      # build a ProgressiveAgentSLM tree from JSON or a Python dict; applies delegate inheritance; binds retrieval_tool ↔ stores
