...
> vector-memory, skills, diagrams, python), and a set of **delegates** — which are themselves
> `ProgressiveAgentSLM` instances.
>
> The agent _progressively_ builds a lightweight **cognitive index** over an **append-only worklog**
> (`raw_worklog.jsonl`, its single source of truth) and retrieves only the blocks it needs back into a
> bounded working window — so quality comes from **disciplined memory handling, not a bigger model**.
> Every agent and delegate reads and writes the same worklog in the run's **worklog_folder**, so
> teammates can loop back over each other's work. Any model slot can be escalated, plug-and-play, to a
> more capable **cloud** model (OpenRouter).
>
...
- **Progressive cognition by indexing, not stuffing.** Each iteration the model's context is
  ...
  piling everything into the prompt, the agent appends its work to an **append-only
  `raw_worklog.jsonl`** and builds a **`cognitive_index`** — a compact map of ~10–20-token pointers
  into that log. To think, it looks up the index and pulls only the relevant blocks back into a
  bounded working set (`context_window.log`). On small models, quality comes from disciplined memory
...
- **One append-only worklog, shared by the whole team.** Every agent and delegate appends finished
  blocks to the same `raw_worklog.jsonl` (the single source of truth) through one serialized writer,
...
> the parent's**. It also shares the parent's `worklog_folder` (hence the shared `raw_worklog.jsonl` +
> `cognitive_index`) while keeping its **own** `context_window.log` + `response_window.log`.
...
prompt, the agent keeps the full record in append-only `raw_worklog.jsonl` and a `cognitive_index`
map over it; the tiers below bound what actually enters the prompt each step.
...
| `current_working_attention`      | 0.525   | The working set for this run: the current user question plus everything retrieved from tools, delegates, and the past worklog (blocks pulled from `raw_worklog.jsonl` via the index).             | Compacted to **50%** when over budget (stale blocks dropped — still recoverable from `raw_worklog.jsonl`).                                          |
| _(remainder ≈ 0.125)_            | —       | The answer the agent emits this iteration (backed by `response_window.log`, §8).                                                                                                                  | Hard output cap = `max_tokens − Σ(declared tiers)`. Flushed to `raw_worklog.jsonl` + indexed, then **cleared** for the next iteration.             |
| _(unbounded)_                    | —       | `raw_worklog.jsonl` — every finished block from every agent / delegate, the **single source of truth**.                                                                                          | **Append-only, never rewritten.** No budget; this is what makes the 50% compactions above safe (nothing is truly lost).                            |
...
```mermaid
flowchart LR
    Q["User / parent<br/>sub-question"] --> R["Retrieve<br/>cognitive_index.search →<br/>raw_worklog.fetch(block_ids)"]
    ...
    T --> W
    D --> W
    W --> F["Flush block →<br/>raw_worklog.jsonl + cognitive_index.jsonl"]
...
per block b produced (delegate answer / tool result / iteration answer):
    block_id ← raw_worklog.append(b)                 # append one JSON line; returns a stable block_id
    cognitive_index.append(pointer(block_id, b))     # {block_id, agent, iter, summary≈10-20 tok, keywords, tokens}

per iteration:
    ids     ← cognitive_index.search(question)        # match pointers (keywords now, embeddings later)
    working ← raw_worklog.fetch(ids)                  # seek blocks by block_id (offset map ⇒ O(1))
    if size(working) + size(cognitive_reflection) > budget:
        reflect_and_compact(target = 0.5 * current_size)  # drop/merge pointers; blocks stay in raw (recoverable)
...
exceed that model's context — no separate size-inference step is needed (§4). Because
`raw_worklog.jsonl` is immutable and `cognitive_index` is a pure **index** (not a compressed blob) that
addresses blocks by stable `block_id`, compaction only ever touches the derived views — the agent can
shrink its working memory aggressively and still recover any detail by following one pointer back into
the raw log.
...
2. **Hands the sub-question down.** The delegate runs its **own** full progressive loop with its own
   `context_window.log` + `response_window.log`, but appends finished blocks to the **shared**
   `raw_worklog.jsonl` + `cognitive_index` (one per run) under its own `agent_id`.
...
The worklog lives in the run's `worklog_folder` (`<worklog_folder>/<run_id>/`). It replaces the old
single `worklog.md`. The design rests on **one clear split**:

- **Shared team memory = structured JSON** (`raw_worklog.jsonl` + `cognitive_index.jsonl`) — durable,
  addressable, and the single source of truth for the whole run.
...
The tiers in §3 are the _prompt-side_ budget; these files are the _on-disk_ storage that budget draws
from.

| File                    | Scope     | Format | Written by                        | Role                                                                                                                                                                                             |
| ----------------------- | --------- | ------ | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `raw_worklog.jsonl`     | shared    | JSONL  | one **serialized writer** per run | **Append-only, never rewritten.** Every finished block (any model, main or delegate) is one JSON record on its own line, keyed by a stable `block_id`. The single source of truth everyone loops back to. |
| `cognitive_index.jsonl` | shared    | JSONL  | the same writer                   | One **pointer record per block**: a ~10–20-token summary + the block's `block_id`. The searchable **map** used to pull only relevant blocks back into a working window.                          |
| `context_window.log`    | per-agent | text   | the owning agent                  | The agent's current working set, streamed to as it works; compacted to 50% when it exceeds the `current_working_attention` budget.                                                             |
| `response_window.log`   | per-agent | text   | the owning agent                  | The agent's **latest** answer only; flushed to `raw_worklog.jsonl` + `cognitive_index.jsonl`, then **cleared** each iteration.                                                                  |

**A "block"** is one unit of finished work — a delegate's answer, a tool result, or an iteration's
answer. Blocks are **flushed at completion** (not token-by-token): the writer buffers the output, then
appends the whole record as one line, so `block_id`s stay unique and parallel delegates never
interleave.

**`raw_worklog.jsonl` block record** (one per line — the verbatim source of truth):
...
2. On block completion, it enqueues the block to the run's **single writer**, which:
   1. appends the block record to `raw_worklog.jsonl` and returns its stable `block_id` (and records a
      `block_id → byte-offset` entry for O(1) fetch);
   2. appends one pointer record to `cognitive_index.jsonl` (a cheap ~10–20-token summary via
      `KeywordExtractor` / `SimpleEntityExtractor`; an LLM summary only when needed).
3. `response_window.log` is cleared for the next iteration.

**The read path (index-and-retrieve).** To assemble its next prompt an agent does **not** replay the
whole log; it queries `cognitive_index` (by keyword, and later by `VectorMemoryTool` embeddings),
takes the matching `block_id`s, and fetches **only those blocks** from `raw_worklog.jsonl` into
`context_window.log`. This is RAG over the team's own worklog — the core trick that keeps SLM prompts
small.

**Progressive reflection (compaction).** When `context_window.log` **+** `cognitive_index` exceed
their budgets (`current_working_attention`, `cognitive_reflection_behavior`), a reflection compacts
**both to 50% of current size**: index pointers are merged / dropped, and stale context blocks are
released. Nothing is lost — `raw_worklog.jsonl` is immutable, so any dropped detail is one
`block_id` lookup away.

**Delegate coordination.** Delegates share this exact behavior. The only difference: a delegate
returns just its **final** answer to its parent, but its full work lands in the shared
`raw_worklog.jsonl` under its `agent_id`, so a later teammate can loop back over it via the index.

```
<worklog_folder>/<run_id>/         # e.g. wip/bvms-assistant/<run_id>/
  raw_worklog.jsonl      # ← shared, append-only source of truth — one JSON block record per line (keyed by block_id)
  cognitive_index.jsonl  # ← shared, one ~10–20-token pointer per block → joins to raw by block_id
  context_window.log     # ← per-agent working set (plain-text scratch; compacts to 50% over budget)
  response_window.log    # ← per-agent latest answer (plain-text scratch; flushed to raw+index, then cleared)
  todo.md                # ← TodoTool checklist (re-injected each iteration)
# <worklog_folder>/index.db        # optional SQLite FTS5 over raw_worklog.jsonl + cognitive_index.jsonl (LogSearch)
```
...
| **Worklog**: one append-only shared source of truth + per-agent working windows | Four-file `worklog_folder` subsystem (`raw_worklog.jsonl` + `cognitive_index.jsonl` shared; `context_window` + `response_window` per-agent)                                       |
...
| Worklog & memory     | **Four-file `worklog_folder` subsystem.** Shared **append-only** `raw_worklog.jsonl` (single source of truth, JSON Lines) + `cognitive_index.jsonl` (pointer map, `block_id`-keyed); per-agent `context_window.log` + `response_window.log` (plain-text scratch). Blocks flush at completion via one serialized writer; **`cognitive` is an index, not a compressed blob**; progressive reflection compacts working views to 50%.                                                       |
| Storage format       | **Append-only JSON Lines** for shared memory (one self-contained block per line, addressed by stable `block_id`) — chosen over text-with-line-ranges (fragile under compaction) and over one monolithic `.json` array (can't append without a rewrite). Enables typed metadata, O(1) fetch via a `block_id → byte-offset` map, and direct FTS5 indexing.                                                                                                                             |
...
src/framework/
  ...
  AgentConfig.py                 # parsed config: agent_id, description, system_prompt, worklog_folder, context_window_breakdown, max_retries_until_switching_models, models[], cognitive_behavior[], tools[], delegates[] (+ inheritance from parent)
  ...
  logging/
    ...
    RawWorklog.py                # append-only raw_worklog.jsonl; append(block) → block_id; maintains a block_id → byte-offset map for O(1) fetch(block_id)
    CognitiveIndex.py            # cognitive_index.jsonl pointer map (block_id-keyed); append(pointer)/search()/compact(0.5)
    ...
    LogSearch.py                 # SQLite FTS5 index (<worklog_folder>/index.db) over raw_worklog.jsonl + cognitive_index.jsonl + search() + CLI
  config/
...
- [~] `logging/` four-file **worklog** subsystem (§8): append-only `RawWorklog` (block append →
  `block_id` + byte-offset) + `CognitiveIndex` (pointer map, `block_id`-keyed, cheap keyword summaries)
  + per-agent `ContextWindowLog` + `ResponseWindow`, coordinated by `Worklog` behind **one serialized
...
- [ ] Progressive reflection: compact `context_window.log` + `cognitive_index` to **50%** when over the
  ...
  stale blocks; recoverable from raw). _(new)_
...
- [~] `AgentConfig.py`: parse `agent_id`, `description`, `system_prompt`, `worklog_folder`,
  `context_window_breakdown`, `max_retries_until_switching_models` (default 5), `models[]`,
  `cognitive_behavior[]`, `tools[]`, `delegates[]`; apply parent→delegate inheritance of the model
  ladder + retry budget + shared `worklog_folder`.
- [~] `ProgressiveAgentSLM.py`: the single recursive class. Per step — retrieve relevant blocks from
  `raw_worklog.jsonl` via `cognitive_index` into `context_window`, assemble the four-tier prompt,
  ...
  the block to `raw_worklog.jsonl` + `cognitive_index.jsonl`, quick self-eval (switch model on repeated
  failure). Recurse into `delegates`; stop when the model ladder is exhausted.
...
- [~] `ToolRegistry.py` + `tools/SupabaseTool.py` (primary; `function_name` + `ranking`) +
  `tools/ReadFileTool.py` + `tools/TodoTool.py`; each tool carries its `when` guidance (used for menu
  pruning).
- [~] Wire `RunLogger` + the four-file `Worklog` subsystem (single serialized writer);
...
- [ ] Remaining tools: `SearchFileTool` + `WriteFileTool` (traversal-safe under `worklog_folder`),
  `VectorMemoryTool` (cross-run memory over a Supabase table), `SkillTool` (progressive-disclosure
  procedure packs), `GenerateDiagramTool` (Mermaid), `RunPythonTool` (wrap `PythonCodeExecute`,
  optional `require_approval`), `FileKnowledgeTool`.
...
- [ ] `logging/LogSearch.py`: SQLite FTS5 index (`<worklog_folder>/index.db`) over `raw_worklog.jsonl`
  + `cognitive_index.jsonl` + search + CLI over all runs.

### Phase 3 — Config loader (JSON + Python) ⬜

- [ ] `config/load.py` + `config/schema.json`: build a `ProgressiveAgentSLM` tree from `example.json`
  (or a Python dict) with validation + delegate inheritance.
- [ ] Round-trip the canonical `example.json` (§13) end-to-end as a worked example + regression check;
  authoring README.

### Phase 4 — Hardening ⬜

- [ ] Unit tests: config loader / inheritance, four-tier budgeting + cascade, `cognitive_index` pointer
  append + 50% compaction (`block_id` integrity), model-ladder switch (single retry budget) +
  success-reset, router description-routing parser, Supabase tool, four-file `Worklog` append / read
  behind one writer, `RunLogger` JSONL + FTS round-trip.
...
retry budget. Both share the parent's `worklog_folder` (one `raw_worklog.jsonl` + `cognitive_index` for
the whole run) while keeping their own `context_window.log` + `response_window.log`. Because every tier
...
> `raw_worklog.jsonl` and mapped in `cognitive_index` (§8), so the two delegates can loop back over each
...
| `all_agent_responses` + `IterationSummarizer` compaction         | Seed for `raw_worklog.jsonl` blocks + the 50% progressive-reflection compaction             |
...
1. **Unit**: config loader + delegate inheritance, four-tier **fractional** budgeting (trim +
   ...
   description-routing selection parser, `SupabaseTool`, four-file `Worklog` append / read (`block_id`
...
2. **Integration smoke**: load `example.json` with a stub model — assert the tree builds, the parent
   ...
   its own `agent_id`, `cognitive_index` grows yet stays ≤ its budget, and `raw_worklog.jsonl` +
   `cognitive_index.jsonl` (+ per-agent `context_window.log` / `response_window.log`) exist and FTS
...
| 2   | Memory model — is `cognitive` compressed knowledge or an index?                                             | An **index** (pointer map into append-only `raw_worklog.jsonl`, addressed by `block_id`); the four-file **worklog** subsystem replaces the old `runs/` artifacts.                                                                                              | ✅ Decided |
...
| 7   | Worklog storage format — text `.log` with line ranges, structured JSON, or a monolithic array?              | **Append-only JSON Lines** (`raw_worklog.jsonl`), one self-contained block per line, addressed by stable `block_id`. Line ranges were fragile under compaction; a `.json` array can't be appended without a rewrite; JSONL keeps append-only, adds typed metadata, works with FTS5, and enables O(1) fetch via a `block_id → byte-offset` map. | ✅ Decided |
...
- **`raw_worklog.jsonl`** — shared, **append-only** single source of truth. Every finished block (any
  model, main or delegate) is one JSON record on its own line, keyed by a stable `block_id`. Never
  rewritten.
- **`cognitive_index.jsonl`** — shared **pointer map**: one record per block (schema below), joined to
  the worklog by `block_id`, used to retrieve only the relevant blocks back into a working window.
- **`context_window.log`** — per-agent working set (plain text); compacted to 50% over its
  `current_working_attention` budget.
- **`response_window.log`** — per-agent latest answer (plain text); flushed to raw + index, then
  cleared each iteration.
- **`<worklog_folder>/index.db`** _(optional)_ — SQLite FTS5 over `raw_worklog.jsonl` +
  `cognitive_index.jsonl` for `LogSearch`.

**`raw_worklog.jsonl` block record** (verbatim block + metadata):
...
**`cognitive_index.jsonl` pointer record** (a **pointer**, not the full block):
...
`router | reflector | tool:NAME | delegate:<agent_id>`. `raw_worklog.jsonl` is the verbatim source of
truth; `cognitive_index.jsonl` is the retrieval map over it (joined by `block_id`); per-agent
`context_window.log` holds only the blocks pulled back in; `response_window.log` holds the current
answer. Compaction only ever touches the derived views — `raw_worklog.jsonl` is immutable, so any
detail is one `block_id` lookup away.
