...
create table n8n_documents_net_micro_neo (
...
BEGIN
  ...
  SELECT
    ...
    summarize,
    metadata,
    2 - ((n8n_documents_net_micro_neo.embedding <=> query_embedding) + (n8n_documents_net_micro_neo.embedding2 <=> query_embedding)) AS similarity
  FROM n8n_documents_net_micro_neo
