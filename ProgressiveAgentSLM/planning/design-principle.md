...
> using: **GitPython** for git history parsing, **langchain-community document loaders** for Confluence /
> Markdown / PDF ingestion, and custom AST parsers (Python `ast`, TypeScript `typescript`) for source
> analysis. Each stage runs as a subprocess under `parallel_subprocesses`; failures in one stage do not
> block others — partial results are staged and retried. Records >4KB are truncated (to fit embedding
> models); stats are logged to `extraction_log.jsonl`.
...
- **Cultivated, layered memory** ([planning §8](planning.md)). Every iteration's raw reasoning is
  ...
  ride in the prompt every step; the rest are pulled on demand through their `retrieval_tool`.
- **Senior-style behavior by policy — fired on hooks, allowed to loop** (`behavior_policies`, [planning §5](planning.md)):
  `deep_planning` decomposes the question, `analyzing_retrieval_results` vets what came back,
  `double_checking` verifies the evidence and — with `circular_behavior_policies_allowed` — can loop back
  up to `behavior_policies_max_circular_rounds`, `visual_representation` emits a Mermaid diagram, and
  `refusing_to_invent` **refuses to invent** when the stores are silent. Because each policy runs **in
  code** at its `run_after` hook, the guardrail against hallucinated "facts" holds even when the SLM
  would rather guess.
- **Prompt-cache discipline** ([planning §3](planning.md)): the run-constant part of the prompt is a
  **byte-stable prefix** the KV cache reuses every iteration; only the retrieved evidence + answer change.
- **Capability-routed model pool + budgets + parallelism** ([planning §2, §4](planning.md)):
  `models_ladder` tags each model by role; `model_selection: "auto"` runs the first general-purpose one
  locally while a cloud model escalates hard steps. **Failover** and **loop bounds** are separate budgets;
  `parallel_subprocesses` (default 1) runs delegate / tool / distillation work sequentially or pooled.
...
[example-revised.json](example-revised.json) (the canonical config). Last updated: 2026-08-12._
